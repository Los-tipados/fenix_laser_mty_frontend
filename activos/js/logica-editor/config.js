export const CONFIG = {
    menu: [
        { id: 'texto', icon: 'A', label: 'Texto' },
        { id: 'productos', icon: '📦', label: 'Productos' },
        { id: 'figuras', icon: '⬔', label: 'Figuras' }
    ],
    // Array de fuentes actualizado con las de Google Fonts + Estándar
    fuentes: [
        /* --- ESTILO URBANO Y RARO --- */
        'Bungee',           // Bloques pesados, muy urbana
        'Permanent Marker',
        
        /* --- ESTILO MODERNO Y TECH --- */
        'Righteous',        // Art-deco moderna y muy legible
        'Orbitron',         // Estilo ciencia ficción / gaming
        'Comfortaa',        // Redonda, amable y muy minimalista
        
        /* --- ESTILO ELEGANTE Y CLÁSICO --- */
        'Abril Fatface',    // Serif de alto contraste (estilo revista de moda)
        'Playfair Display', // Elegancia pura
        'Fascinate Inline', // Estilo Broadway/Cine antiguo
        
        /* --- ESTILO DIVERTIDO Y RETRO --- */
        'Pacifico',         // Script/Cursiva de los años 50 suave
        'Press Start 2P',   // Pixel art de consola de 8 bits
        'Montserrat',       // La reina de la versatilidad moderna
        
        /* --- BÁSICAS --- */
        'Arial'
        
    ],
    productos: [
        { id: 'p1', nombre: 'Caja', url: 'https://i.ibb.co/JwT7G5nb/box-real.webp'},
        { id: 'p2', nombre: 'Cuchillo', url: 'https://i.ibb.co/WvXx1qW6/box.webp' },
        { id: 'p3', nombre: 'Tabla redonda', url: 'https://i.ibb.co/8gXXPtfV/imagen1.webp' },
        { id: 'p4', nombre: 'Reloj', url: 'https://i.ibb.co/J9cwXtV/reloj.webp' },
        { id: 'p5', nombre: 'Tabla', url: 'https://i.ibb.co/W4SMNZKN/tabla.webp' }
    ],
    figuras: [
        { nombre: ' ', url: 'https://i.ibb.co/QjjGXwpV/animal-1295493-1280.png' },
        { nombre: ' ', url: 'https://i.ibb.co/BHk0rYYd/panda.png' },
        { nombre: ' ', url: 'https://i.ibb.co/PGRR9QZ3/Corazon.png' },
        { nombre: ' ', url: 'https://i.ibb.co/MkYrL1jv/horse-3254102-1280.png' },

        { nombre: ' ', url: 'https://i.ibb.co/M5DR24PT/carro-oldie.png'},
        
        { nombre: ' ', url: 'https://i.ibb.co/DZH52CB/marco1.png'},
        
        { nombre: ' ', url: 'https://i.ibb.co/4R762cYd/corona.png'},
        { nombre: ' ', url: 'https://i.ibb.co/FL1gyMNj/corazoncitos.png'},
        { nombre: ' ', url: 'https://i.ibb.co/bgNPX1wM/ai-generated-9166476-1280.png'},
        { nombre: ' ', url: 'https://i.ibb.co/Kcv62TFt/divider-9999292-640.png'},
        
        { nombre: ' ', url: 'https://i.ibb.co/4nDSX19S/espada.png'},
        { nombre: ' ', url: 'https://i.ibb.co/S7BSPnkj/ilustracion.webp'}
    ]
};
